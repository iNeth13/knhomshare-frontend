...
export default function AuthorProvider({ children }) {
  ...
  const handleFollowAuthor = async (userId, userToken, authorId, type) => {
    try {
      dispatch({ type: AUTHOR_GET_FOLLOW_REQ });
      const response = await fetch(
        `${process.env.REACT_APP_DEFAULT_URL}/api/author/${type}`,
        {
          headers: {
  ...
  return (
    <authorContext.Provider
      value={{ ...state, handleRecommendAuthor, handleFollowAuthor }}
    >
      {children}
    </authorContext.Provider>
  );
